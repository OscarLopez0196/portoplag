...
from cv2 import cv2  #para uso de opencv, tratamiento de imágenes.
# flask --> framework de Python que permite crear aplicaciones web rápidamente y con un mínimo número de líneas de código.
# flask_mysqldb --> para uso de MYSQL.
# render template --> para uso de plantillas HTML en Python.
# abort --> en casos de errores.
# Response --> respuesta del servidor local
# jsonify --> para uso de archivos tipo JSON.
# redirect --> para redireccionamiento a plantillas o funciones.
# url_for --> para insertar funciones y poder direccionar al usuario.
# session --> variable global de sesión para el usuario.
# request --> para petición al servidor, en caso de formularios.
from flask import Flask, abort, render_template, Response, jsonify, redirect, url_for, session, request
...
def login():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        cursor.execute('SELECT * FROM cuenta WHERE username = %s AND pass = %s', (username, password))
...
def registro():
    ...
    msg = ''
    # mensaje de salida para error de password
    msg_password = ''
    ...
    if request.method == 'POST' and 'name' in request.form and 'second' in request.form and 'username' in request.form and 'password' in request.form  and 'c_password' in request.form:
        ...
        cursor.execute('SELECT * FROM cuenta WHERE username = %s', (username,))
        ...
        # validación si los datos del formulario no fueron rellenados.
            ...
            msg_password = 'Las contraseñas no coinciden - Vuelva a insertar sus datos'
            ...
            cursor.execute('INSERT INTO cuenta VALUES (NULL, %s, %s, %s, %s, %s)', (name, second, username,password, c_password))
